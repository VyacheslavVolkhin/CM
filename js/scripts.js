document.addEventListener("DOMContentLoaded", function() {
	...
	//datepicker
	flatpickr(".frm-field-input.field-date input", {
		firstDayOfWeek: 0,
		disableMobile: "true",
		minDate: "today",
		dateFormat: "Y-m-d",
	...
	flatpickr(".frm-field-input.field-date-period input", {
		firstDayOfWeek: 0,
		disableMobile: "true",
		mode: "range",
		minDate: "today",
	...
	//select style
	document.querySelectorAll('.frm-field-input select').forEach(function(select) {
		new Choices(select);
	});
	...
	for (let i = 0; i < passwordToggle.length; i++) {
	  passwordToggle[i
		...
		if (this.classList.contains("active")) {
		  ...
		  const input = this.closest(".field-password").querySelector(
			".form-input"
		  );
		  input.type = "password";
			} else {
